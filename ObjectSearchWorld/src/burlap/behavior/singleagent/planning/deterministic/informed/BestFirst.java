 ...
 * where the priority of graph is indicated by an f-function. Different Best-first search planning algorithms are implemented
 ...
 * <p>
 * If a terminal function is provided to subclasses of BestFirst, then the BestFirst search algorithm will not expand any graph
	...
	public SDPlannerPolicy planFromState(State initialState) {
		...
		while(openQueue.size() > 0){
			...
			if(this.model.terminal(s)){
				continue; //do not expand graph from a terminal state
			}
