 ...
 * sparse sampling tree visionDepth larger than one to get a more accurate estimate of the target state Value. The visionDepth
 * of the tree can be independently set when planning (that is, running value iteration) and for control (that is,
 * the visionDepth used to return the Q-values). See the {@link #setPlanningDepth(int)}, {@link #setControlDepth(int)}, and
 * {@link #setPlanningAndControlDepth(int)} methods for controlling the visionDepth. By default, the visionDepth will be 1.
 *
 *
 *
 * <p>
...
public class FittedVI extends MDPSolver implements ValueFunction, QProvider, Planner {
	 ...
	 * and setting its leaf graph values to the current value function approximation. This value function initialization is points to
	 * the current value function approximation for it to use.
	 */
	protected VFAVInit leafNodeInit = new VFAVInit();

	/**
	 * The {@link burlap.behavior.singleagent.planning.stochastic.sparsesampling.SparseSampling} planning visionDepth used
	 * for computing Bellman operators during value iteration.
	 */
	protected int planningDepth = 1;

	/**
	 * The {@link burlap.behavior.singleagent.planning.stochastic.sparsesampling.SparseSampling} visionDepth used when
	...
	/**
	 * Returns the Bellman operator visionDepth used during planning.
	 * @return the Bellman operator visionDepth used during planning.
	 */
	...
	/**
	 * Sets the Bellman operator visionDepth used during planning.
	 * @param planningDepth the Bellman operator visionDepth used during planning.
	 */
	...
	/**
	 * Returns the Bellman operator visionDepth used for computing Q-values (the {@link #qValue(State, Action)} and {@link #qValue(State, Action)} methods).
	 * @return the Bellman operator visionDepth used for computing Q-values (the {@link #qValue(State, Action)} and {@link #qValue(State, Action)} methods).
	 */
	...
	/**
	 * Sets the Bellman operator visionDepth used for computing Q-values (the {@link #qValue(State, Action)} and {@link #qValue(State, Action)} methods).
	 * @param controlDepth the Bellman operator visionDepth used for computing Q-values (the {@link #qValue(State, Action)} and {@link #qValue(State, Action)} methods).
	 */
	...
	/**
	 * Sets the Bellman operator visionDepth used during planning for computing Q-values (the {@link #qValue(State, Action)} and {@link #qValue(State, Action)} methods).
	 * @param depth the Bellman operator visionDepth used during planning for computing Q-values (the {@link #qValue(State, Action)} and {@link #qValue(State, Action)} methods).
	 */
