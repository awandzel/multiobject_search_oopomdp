 ...
 * method to ensure this, however, the required horizon and Y will probably be intractably large). SS must replan for every new state it sees, so an agent following it in general must replan after every step it takes in the real world. Using a
 ...
 * <p>
 * The algorithm operates by building a tree from the source initial state. The tree is built by sampling Y outcome states for each possible state-action pair, thereby generating new
 * state nodes in the tree. The tree is built out to a fixed height H and then in a tail recursive way, the Q-value and state value is estimated using a Bellman update as if the Y samples perfectly defined
 ...
 * <p>
 * By default, the Y parameter (number of state transition samples) is fixed for all nodes; however, it may also be set to use a variable Y that reduces the number
 * of sampled states the further down in the tree it is according to C_i = C_0 * gamma^(2i), where i is the visionDepth of the node from the root and gamma is the discount
 * factor.
 * <p>
...
public class SparseSampling extends MDPSolver implements QProvider, Planner {
  ...
  /**
   * The number of transition dynamics samples (for the root if visionDepth-variable Y is used)
   */
  protected int c;


  /**
   * Root node for the purposes of walking though reproduction of the BSS sampling tree.
   * by Arthur Wandzel
   */
  public Node rootDebugTraceNode = null;

  /**
   * Enables BSS sampling tree reproduction; default is not (false).
   * by Arthur Wandzel
   */
  public boolean debugTraceNode = false;

  /**
   * Sets debugTraceNode
   * by Arthur Wandzel
   */
  public void setDebugTraceNode(boolean b) {this.debugTraceNode = b;}

  /**
   * Whether the number of transition dyanmic samples should scale with the visionDepth of the node. Default is false.
   */
  ...
  public void setHAndCByMDPError(double rmax, double epsilon, int numActions) {
    ...
    DPrint.cl(this.debugCode, "Y = " + this.c);
  }


  /**
   * Sets whether the number of state transition samples (Y) should be variable with respect to the visionDepth of the node. If set
   * to true, then the samples will be defined using C_i = C_0 * gamma^(2i), where i is the visionDepth of the node from the root, gamma is the discount factor
   * and C_0 is the normal Y value set for this object.
   *
   * @param useVariableC if true, then visionDepth-variable Y will be used; if false, all state nodes use the same number of samples.
   */
  ...
  public GreedyQPolicy planFromState(State initialState) {
    ...
    if (this.rootLevelQValues.containsKey(sh)) {
      //System.out.println("Already planned from this state");
    ...
    if (debugTraceNode) {
      this.rootDebugTraceNode = new Node(); //node for walking through BSSSamplingTree
      rootLevelQValues.put(sh, sn.estimateQs(this.rootDebugTraceNode));
    } else {
  ...
  public List<QValue> qValues(State s) {
    ...
    if (qs == null || this.forgetPreviousPlanResults) {
  ...
  /**
   * Returns the value of Y for a node at the given height (height from a leaf node).
   *
   * @param height the height from a leaf node.
   * @return the value of Y to use.
   */
  protected int getCAtHeight(int height) {
    ...
    //convert height from bottom to visionDepth from root
    int d = this.h = height;
  ...
  public class StateNode {
    ...
    protected double sampledQEstimate(Action ga) {
      ...
      for (int i = 0; i < c; i++) {
        ...
        sum = Math.round(sum * 1e6) / 1e6; //round total reward to account for floating-point rounding errors
      }
      sum /= (double) c;
    ...
    protected double exactQValue(Action ga) {
      ...
      if (!(ga instanceof Option)) {
        ...
        throw new RuntimeException("Sparse Sampling Planner with Full Bellman updates turned on cannot work with options because it needs factored access to the visionDepth for each option transition. Use the standard sampling mode instead.");
      }
    ...
    /**
     * For debugging purposes : for BSS sampling tree reproduction.
     */
    public List<QValue> estimateQs(Node n) {
      List<Action> gas = SparseSampling.this.applicableActions(this.sh.s());
      List<QValue> qs = new ArrayList<QValue>(gas.size());
      for (Action ga : gas) {

//				if (ga.actionName().equals("look") && this.height == 3){
//					int i =1;
//				}
        ...
        if (this.height <= 0) {
          qs.add(new QValue(this.sh.s(), ga, SparseSampling.this.vinit.value(this.sh.s())));
          nChild.r = qs.get(qs.size() - 1).q;
          nChild.children = null;
        } else {
          double q;
          if (!SparseSampling.this.computeExactValueFunction) {
            q = this.sampledQEstimate(ga, nChild);
          } else {
            q = this.exactQValue(ga, nChild);
          }
     ...
     * For debugging purposes : for BSS sampling tree reproduction.
     */
    protected double sampledQEstimate(Action ga, Node n) {
      double sum = 0.;
      ...
      for (int i = 0; i < c; i++) {
        ...
        StateNode nsn = SparseSampling.this.getStateNode(ns, this.height - k);
        n.hhs = new HashedHeightState(nsn.sh, this.height - k);
        n.ns = ((EnumerableBeliefState.StateBelief) ((TabularBeliefState) nsn.sh).nonZeroBeliefs().toArray()[0]).s;

        sum += r + Math.pow(SparseSampling.this.gamma, k) * nsn.estimateV(n);
        sum = Math.round(sum * 1e6) / 1e6; //round total reward to account for floating-point rounding errors
      }
      sum /= (double) c;
     ...
     * For debugging purposes : for BSS sampling tree reproduction.
     */
    protected double exactQValue(Action ga, Node n) {
      double sum = 0.;
      List<TransitionProb> tps = ((FullModel) model).transitions(sh.s(), ga);
      ...
      if (!(ga instanceof Option)) {
        ...
        for (TransitionProb tp : tps) {
          double r = tp.eo.r;
          StateNode nsn = SparseSampling.this.getStateNode(tp.eo.op, this.height - 1);
          n.hhs = new HashedHeightState(nsn.sh, this.height - 1);
          n.ns = ((EnumerableBeliefState.StateBelief) ((TabularBeliefState) nsn.sh).nonZeroBeliefs().toArray()[0]).s;

          sum += tp.p * (r + SparseSampling.this.gamma * nsn.estimateV(n));
          sum = Math.round(sum * 1e6) / 1e6; //round total reward to account for floating-point rounding errors
        }
     ...
     * For debugging purposes : for BSS sampling tree reproduction.
     */
    public double estimateV(Node n) {
      if (this.closed) {
        return this.v;
      }
      ...
      List<QValue> Qs = this.estimateQs(n);
      double[] qs = new double[Qs.size()];
      for (int i = 0; i < Qs.size(); i++) {
        qs[i] = Qs.get(i).q;
      }
  ...
  /**
   * Contains information necessary for debugging purposes
   *
   * @author Arthur Wandzel
   */
  public static class Node {
    HashedHeightState hhs;
    State ns;
    Action a;
    double r;
