...
public class UCT extends MDPSolver implements Planner, QProvider {
	...
	/**
	 * Performs a rollout in the UCT tree from the given node, keeping track of how many new graph can be added to the tree.
	 * @param node the node from which to rollout
	 * @param depth the visionDepth of the node
	 * @param childrenLeftToAdd the number of new subsequent graph that can be connected to the tree
	...
	public double treeRollOut(UCTStateNode node, int depth, int childrenLeftToAdd){
		...
		if(model.terminal(node.state.s())){
			...
			DPrint.cl(debugCode, numRollOutsFromRoot + " Hit terminal at visionDepth: " + depth);
			return 0.;
		}
		...
		if(snprime != null){
			...
			//this state is not in the tree at this visionDepth so create it
			snprime = stateNodeConstructor.generate(shprime, depth+1, actionTypes, actionNodeConstructor);
			
			//store it in the tree depending on how many new graph have already been stored in this roll out
	...
	/**
	 * Returns the {@link UCTStateNode} for the given (hashed) state at the given visionDepth.
	 * @param sh the state whose node should be returned
	 * @param d the visionDepth of the state
	...
	protected UCTActionNode bestReturnAction(UCTStateNode snode){
		...
		for(UCTActionNode anode : snode.actionNodes){
			...
			//only select graph that have been visited
