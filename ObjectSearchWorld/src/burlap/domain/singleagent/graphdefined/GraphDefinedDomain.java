 ...
 * <p>
 * A constructed nodes's transition dynamics can be validated as proper (all nodes from a state-action pair sum to 1)
 * using the method {@link #isValidMDPGraph()} and if they are false, a string reporting which state-actions do not
 * sum to 1 (and their various graph) can be returned with the {@link #invalidMDPReport()} method.
 * <p>
...
public class GraphDefinedDomain implements DomainGenerator {
	...
	/**
	 * The number of state nodes in the nodes
	 */
	...
	/**
	 * Initializes the generator to create a domain with the given number of state graph in it.
	 * @param numNodes the number of state graph in the domain.
	 */
	...
	/**
	 * Returns the number of state graph specified in this domain
	 * @return the number of state graph specified in this domain
	 */
	 ...
	 * Note that this method also defines from which graph an action can be executed. If this method is never called for source node i and action j, then
	 * it will be assumed that action j cannot be executed in state node i. The the specified source node or target node
	 * is greater than this domain generator's previously specified number of graph, then space will be added for them
	...
	/**
	 * Clears all (stochastic) graph for a given state-action pair.
	...
	/**
	 * An action class for defining actions that can be taken from state graph. The action can only be taken
