...
public class OOOPOMCP {
  ...
  public OOOPOMCP(Random rn, PODomain pd, int sim, int h, double c, double d, boolean r, int ps, objectDistribution fbe) {
    this.DOMAIN = pd;
  ...
  public Action SelectAction(VNode root) {
    for (int i = 0; i < OOOPOMCP.SIMULATIONS; i++) {
      OOOPOMCP.DEPTH = 0;
      SearchState s = root.B.get(rnPomcp.nextInt(root.B.size()));
      root.SimulateV(s);
    }
    if (printDepth) System.out.println(OOOPOMCP.MAX_DEPTH_REACHED);
...
//  }
  ...
  public VNode Update(SearchState currentState, Action a, State o, VNode vn) {
    ...
    List<SearchState> newBelief = objectDistribution.POMCPSample(currentState, OOOPOMCP.SIMULATIONS);
...
//      }
      ...
      for (QNode qn : this.children.values()) {
        ...
        if (ucb) {
          if (qn.V.size() == 0)
            ...
            qvalue += OOOPOMCP.EXPLORATION * Math.sqrt((Math.log(this.V.size() + 1) / qn.V.size()));
        }
    ...
    Double SimulateV(State s) {
      if (OOOPOMCP.DEPTH >= OOOPOMCP.MAX_DEPTH) {
        return 0.0;
      }
...
//      }
    ...
    Double SimulateQ(State s, QNode qn, Action a) {
      ...
      EnvironmentOutcome eo = OOOPOMCP.DOMAIN.getModel().sample(s, a);
      State o = OOOPOMCP.DOMAIN.getObservationFunction().sample(eo.op, a);
      visionConeObservation op = (visionConeObservation) o;

      VNode vn = qn.children.get(op);
      //use only what is sampled not whole space
      if (vn == null) {
        qn.children.put(op, new VNode(a, op, 0.0));
        vn = qn.children.get(op);
      }

      if (!eo.terminated && vn.children.isEmpty() && qn.V.size() >= 1) {
        vn = ExpandNode(eo.op, a, op);
        qn.children.put(op, vn);
      }

      if (!eo.terminated) {
        OOOPOMCP.DEPTH++;
        if (OOOPOMCP.DEPTH > MAX_DEPTH_REACHED) MAX_DEPTH_REACHED = OOOPOMCP.DEPTH; //debug code
      ...
      double totalReward = eo.r + OOOPOMCP.DISCOUNT * delayedReward;
    ...
    double issueRollout(State s, double r, int depth, double discount, boolean terminated) {
      if (depth >= OOOPOMCP.MAX_DEPTH_RANDOM_ROLLOUT || terminated) {
        return r;
      }
